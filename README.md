...
## 特徴
- 主要コードは MicroPython で書かれたプログラマブル水位計です。目的に応じて測定間隔など様々なパラメータを変更することができます。プログラムの変更も自由です。
- 1台約6,000円と安価に作成できます。（部品の送料、消耗品、工具等の費用を除く）
- Raspberry Pi Pico 内のメモリをロガーとしてcsv形式で計測値（距離、気温）を記録することができます。
- 超音波センサーの性能に基づき、0.3cm 程度の分解能の計測が可能です。（測定距離min: 0.02m、測定距離max: 4.5m、動作温度min: -10℃、動作温度max: 70℃、有効角度： 15度）　超音波の速度について温度補正を行っています。
