...
Raspberry Pi Pico を接続後、「ファイル」-「Open」を選択して、画面下の「Raspberry Pi Pico」を押してください。但し、**既に main.py がある場合は「ヘルプ」の下にある赤い「STOP」ボタンを押してください。通電すると自動的に測定モードに入ってしまうためプログラムを停止する必要があります。**
