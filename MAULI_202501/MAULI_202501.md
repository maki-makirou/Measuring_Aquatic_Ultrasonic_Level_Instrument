...
コードを修正する際は、Thonny でパラメータ `SAVING_MODE = 0` にしてください。
これは2箇所ある省電力モード指定を lightsleepモードにするか sleepモードにするか選択するパラメータです。
`SAVING_MODE = 0`にすると sleepモードになり、通常の sleep命令を実行します。
`SAVING_MODE = 1`にすると消費電力が少ない lightsleep命令を実行します。
