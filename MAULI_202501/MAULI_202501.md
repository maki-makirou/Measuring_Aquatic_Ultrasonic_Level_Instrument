...
コードを修正する際は、Thonny で下記の通り2箇所ある`machine.lightsleep`をコメントアウトして、`utime.sleep`をコメント解除してからデバッグしてください。
`machine.lightsleep`が実行されている間は赤色の「STOP」ボタンを受け付けなくなったり、PC が Raspberry Pi Pico を見失ってエラーが出たりするためです。
`machine.lightsleep`は待機電力を少なくするための重要なスリープ命令です。デバッグが終わったら、`machine.lightsleep`をコメント解除して、`utime.sleep`をコメントアウトして元に戻してください。
