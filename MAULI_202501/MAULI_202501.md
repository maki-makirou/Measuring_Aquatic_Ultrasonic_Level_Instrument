...
Raspberry Pi Pico を接続後、画面下の「Raspberry Pi Pico」を押してください。但し、**既に main.py がある場合は「ヘルプ」の下にある赤い「STOP」ボタンを押して自動的に入ってしまう測定モードを止める必要があります。つまり初回以外は、常に接続後「ヘルプ」の下にある赤い「STOP」ボタンを押す必要があるので注意してください。**
...
main.py を書き込んでください。正常な状態の場合、画面のようになります。**但し、初回は log.csv はありません。一度計測すると log.csv が現れます。** 
...
PCから外しバッテリーを接続して、うまく動くとこのようになります。
